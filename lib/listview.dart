...
class _lhotelState extends State<lhotel> {
  ...
  Future<ListHotel> getListHotel() async{
    ...
    var dio = Dio();
    Response response = await dio.get('https://dev.farizdotid.com/api/purwakarta/hotel');
    print(response.data);
    if(response.statusCode == 200 ){
...
//                    ),
                      child: ListTile(
                        leading: CircleAvatar(
                            radius: 70, //liat di figma roundednya
                            backgroundImage : NetworkImage('${snapshot.data.hotel[index].gambarUrl}')),
...
class _lkulinerState extends State<lkuliner> {
  ...
  Future<ListKuliner> getListKuliner() async{
    ...
    var dio = Dio();
    Response response = await dio.get('https://dev.farizdotid.com/api/purwakarta/kuliner');
    print(response.data);
    if(response.statusCode == 200 ){
...
//                    ),
                      child: ListTile(
                        leading: CircleAvatar(
                            radius: 70, //liat di figma roundednya
                            backgroundImage : NetworkImage('${snapshot.data.kuliner[index].gambarUrl}')),
